  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* Vet Info Header */}
        <div className="flex items-center gap-4 pb-4 border-b border-gray-200 dark:border-[var(--app-border-default)]">
          <div className="w-16 h-16 bg-primary-100 dark:bg-primary-900/30 rounded-full flex items-center justify-center">
            <UserCircleIcon className="w-10 h-10 text-primary-600 dark:text-primary-400" />
          </div>
          <div>
            <h3 className="text-xl font-semibold text-gray-900 dark:text-[var(--app-text-primary)]">{vet.vetName}</h3>
        ...
        <div className={`p-4 rounded-lg ${getPerformanceBgColor(vet.completionRate)}`}>
          <div className="flex items-center justify-between mb-2">
            <span className="font-medium text-gray-700 dark:text-[var(--app-text-secondary)]">{t('vetPerformance.completionRate')}</span>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="p-4 bg-gray-50 dark:bg-[var(--app-bg-tertiary)] rounded-lg text-center">
            <DocumentTextIcon className="w-8 h-8 mx-auto mb-2 text-gray-400" />
            <p className="text-2xl font-bold text-gray-900 dark:text-[var(--app-text-primary)]">{vet.totalRecords}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">{t('vetPerformance.totalRecords')}</p>
          </div>
          <div className="p-4 bg-green-50 dark:bg-green-900/30 rounded-lg text-center">
            <CheckCircleIcon className="w-8 h-8 mx-auto mb-2 text-green-500 dark:text-green-400" />
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{vet.completeRecords}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">{t('vetPerformance.complete')}</p>
          </div>
          <div className="p-4 bg-red-50 dark:bg-red-900/30 rounded-lg text-center">
            <XCircleIcon className="w-8 h-8 mx-auto mb-2 text-red-500 dark:text-red-400" />
            <p className="text-2xl font-bold text-red-600 dark:text-red-400">{vet.incompleteRecords}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">{t('vetPerformance.incomplete')}</p>
          </div>
        </div>
